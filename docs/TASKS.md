...
## In Progress
- [ ] M62b: `replan_history` へ entry を追記できる helper を追加する
  - Title: replan history append helper を追加する
  - Milestone: M62 Replan Loop
  - Purpose: replan loop 本体より先に、future chapter 再計画の履歴を fail-fast に追記できる storage helper を整える
  - Target files or directories: `src/novel_writer/storage.py`, `tests/test_storage.py`, `README.md`, `docs/TASKS.md`, `docs/ROADMAP.md`
  - Done when: `replan_history` が未作成でも新規生成でき、既存履歴には `replan_id` 単位で追記または置換できる helper が tests / docs で固定される
