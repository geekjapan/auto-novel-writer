...
- `show-run-comparison` の read-only 経路は、最小 valid comparison artifact と zero-candidate artifact に対しても tests で固定されている
...
- comparison artifact から current / best run の `output_dir` も確認できる
- comparison artifact から `run_candidates[*].run_name` も確認できる
- comparison artifact から `run_candidates[*].score` も確認できる
- comparison artifact から `run_candidates[*].output_dir` も確認できる
...
- `show-run-comparison` の minimal valid artifact ケースでは、`best_selection_source`、`best_selection_reason_codes`、`current_comparison_reason_codes`、`current_comparison_metrics`、`best_comparison_metrics`、read-only 性まで tests で固定する
- `run_candidates=[]` のケースでは `Run candidates: 0` は表示しつつ、candidate detail line を出さない挙動を tests で固定する
...
- README / tests に status summary field と artifact field の対応表がある
- `show-project-status` の builder が manifest 読込と分離され、単体テスト可能になっている
- docs / tests がその対応関係を固定する
- `show-project-status` と `show-run-comparison` の使い分けが README / ROADMAP / TASKS で明確になる
- minimal valid comparison artifact の read-only 表示について、current / best / compact / candidate count の必須行と zero-candidate 境界が tests で固定される

## 次段階

- `show-run-comparison` の minimal valid artifact coverage を best metrics から compact 行まで順に固定する
- `run_candidates=[]` の minimal artifact read-only 経路では、`compact.issue_score` と `compact.completed_step_count` に続いて `compact.long_run_should_stop` の表示も tests で固定する
- comparison artifact の read-only 経路で、mandatory line と optional line の境界を tests で明示する
- comparison CLI の machine-readable contract と human-readable line 出力の対応を、README / ROADMAP / TASKS で同じ言葉に揃え続ける
