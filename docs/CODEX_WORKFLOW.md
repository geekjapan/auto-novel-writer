...
- `ROADMAP.md`: 現在地と次段階
- `DEVELOPMENT_GUIDE.md`: 優先順位と task 分解基準
- `README.md`: 利用者向け現状仕様
...
4. 現在タスクの完了条件を確認し、repository 内の推奨値、既定値、既定フローが読める場合はそれを採用して、必要最小限の変更だけを入れる
...
9. 小さなコミットを作る
10. 停止条件に該当しない限り、ユーザーの追加指示や都度確認を待たずに次のループへ進む
...
13. milestone が大きすぎる、または task 境界が曖昧な場合は、停止ではなく先に task を再分割して再計画する
14. 停止条件に該当する場合だけ停止し、`docs/BLOCKED.md` を更新する
...
- repository 内に推奨値や既定方針がある場合、Codex はそれを採用し、軽微な設計確認のためには停止しない
- 書かれた implementation plan があり、実行方式の選択が必要な場合は、既定で Subagent-Driven を選ぶ。別方式はユーザーが明示したときだけ採る
...
- 推奨方針を採用しても、現在タスクの受け入れ条件が解決できず、安全なコード仕様の判断ができない
- 既存ユーザーの利用方法、既存 CLI、artifact contract、schema version、保存形式を大きく壊す可能性が高い
- どの挙動を正とするか、実装と docs のどちらを基準にすべきか、repository 内の推奨値を使っても判断できない
- 作業中にユーザー変更と衝突する差分を見つけ、そのまま進めると破綻や上書きの危険が高い
- 既存 validator / schema contract が task 文面と矛盾し、推奨方針でも安全側に倒せない
- 継続すると、データ破壊、復旧困難、広範囲 migration のような高リスク事象につながる可能性が高い

停止の前に、次に当てはまる場合は task の再分割または再計画を先に試す。

- 1 milestone が大きすぎて単一 task へ安全に落とせない
- テストを書こうとすると責務境界が曖昧になる
- 同じ入力資産を複数 artifact が重複保持し始める
- resume / rerun / comparison の整合が崩れる
...
- 互換維持より整理を優先すべきか決められず、推奨方針でも安全に決められない
- タスクが大きすぎて分割方針に複数の妥当案があり、どれを選んでも互換性や運用に大きく影響する
- OpenAI API 利用方針や評価軸のように、repository 内の推奨値では代替できないプロダクト判断が必要
