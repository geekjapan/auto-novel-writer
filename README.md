...
`replan_history` は、future chapter の再計画履歴を追跡する machine-readable artifact として schema / storage contract を導入しました。現在の contract は top-level に `schema_name`, `schema_version`, `replans` を持ちます。各 replan entry は `replan_id`, `trigger_chapter_number`, `reason`, `issue_codes`, `impact_scope`, `updated_artifacts`, `change_summary` を required field に固定しています。`impact_scope` は `from_chapter`, `to_chapter`, `chapter_numbers` を持ち、どの章範囲へ影響した replan かを明示します。現在は contract のみが導入済みで、次段階で append helper と実際の replan loop を接続します。
