...
saved `next_action_decision` では、次の処理方針そのものは `action`、短い理由は `reason`、補足根拠は `decision_trace` に保存されます。
比較表示で見える `reason_detail` 系の語彙は、`next_action_decision` 本体ではなく comparison / selection context 側の read-only 表示です。
`show-project-status` は saved `next_action_decision` を読むものの、`decision_trace` 全文を展開する場ではなく、resume gate や保存済み snapshot を確認するための read-only view です。
saved `next_action_decision` に `story_state_summary` が含まれている場合、`show-project-status` は current run に加えて best run でも、その保存済み story-state snapshot を `saved_story_state_summary` 行として read-only に表示できます。
`show-run-comparison` は current / best run の比較理由、`reason_detail` 系の語彙、saved `next_action_decision.story_state_summary` を並べて見る read-only view です。
...
`story_state_summary` は `progress_report` を起点に保存され、saved `next_action_decision`、`replan_history`、`publish_ready_bundle.summary` へ再利用されます。
`show-project-status` と `show-run-comparison` は、その保存済み snapshot を read-only に表示します。
publish bundle 側の read-only 表示では `publish_bundle.story_state_summary` という行ラベルを使います。
`show-project-status` は `autonomy_level`、resume gate、saved `next_action_decision` の snapshot を見る場で、`show-run-comparison` は current / best run の比較理由、`reason_detail` 系の表示、saved summary を並べて見る場です。
...
- `docs/CODEX_WORKFLOW.md`: 実装ループと停止条件
