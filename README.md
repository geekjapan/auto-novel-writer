...
`replan_history` は、future chapter の再計画履歴を追跡する machine-readable artifact として導入済みです。現在の contract は top-level に `schema_name`, `schema_version`, `replans` を持ちます。各 replan entry は `replan_id`, `trigger_chapter_number`, `reason`, `issue_codes`, `impact_scope`, `updated_artifacts`, `change_summary` を required field に固定しています。`impact_scope` は `from_chapter`, `to_chapter`, `chapter_numbers` を持ち、どの章範囲へ影響した replan かを明示します。`chapter_numbers` は `from_chapter..to_chapter` の連番と一致しない場合に fail fast で停止します。storage helper では `replan_id` を主キーにした upsert ができ、未作成 history からの新規生成、既存 replan の置換、新規 replan の追記を fail-fast の validator つきで扱えます。さらに `apply_replan_updates()` により、既存の `chapter_briefs.json` / `scene_cards.json` を読み込んだうえで、impact scope に含まれる future chapter だけを置換できます。この helper は past / current chapter への更新、impact scope と payload の章番号不一致、既存 artifact 範囲外の更新を fail fast で拒否します。現在の pipeline では `progress_report.recommended_action == "replan"` を検出すると、`replan_history.json` へ decision trace を 1 entry 保存し、future chapter が残っている場合は再生成した `chapter_briefs` / `scene_cards` の future 範囲だけを自動適用します。apply 後の `change_summary` には、artifact ごとの更新章要約も追記されます。
...
`next_action_decision` は、M63 の自律判断層に向けた machine-readable artifact です。現在の contract は `schema_name`, `schema_version`, `evaluated_through_chapter`, `action`, `reason`, `issue_codes`, `target_chapters`, `policy_budget`, `decision_trace` を持ちます。`action` は `continue`, `revise`, `rerun_chapter`, `replan_future`, `stop_for_review` の列挙型に固定しています。`policy_budget` には `max_high_severity_chapters`, `max_total_rerun_attempts`, `remaining_high_severity_chapter_budget`, `remaining_rerun_attempt_budget` を保存します。`decision_trace` は `code`, `summary`, `value` を持つ object 配列で、判断根拠を machine-readable に残します。save/load helper でも同じ contract を検証します。現在の pipeline では `progress_report.json` 保存直後に `next_action_decision.json` も保存し、`progress_report.recommended_action` を `next_action_decision.action` へ明示マッピングしたうえで、long-run budget と各 check の `status` / `summary` を machine-readable に引き継ぎます。`continue`, `revise`, `rerun`, `replan`, `stop_for_review` の全列挙型 mapping は pipeline tests で固定しており、`replan` の場合は future chapter 範囲、`revise` / `rerun` の場合は評価対象章を `target_chapters` に保存します。validator は action ごとの `target_chapters` ルールも fail-fast に検証し、`continue` / `stop_for_review` では空配列、`revise` / `rerun_chapter` では単一章、`replan_future` では 1 件以上を必須にします。
